...
def create_permutation_instruction_chat(query: str, docs: List[SearchResult], model_name='gpt-3.5-turbo'):
    ...
    while True:
        ...
        for doc in docs:
            ...
            # For API-based models we may still need dynamic shortening to fit
            # context limits. For local models (model_name=None), keep full text.
            words = content.split()
            if model_name is not None:
                content = ' '.join(words[:int(max_length)])
            else:
                content = ' '.join(words)
...
class OpenAiListwiseLlmRanker(LlmRanker):
    def __init__(self, model_name_or_path, api_key, window_size, step_size, num_repeat):
        ...
        # Per-query trace filled in rerank(); consumed by outer pipeline logs.
        self.last_compare_trace = []
    ...
    def rerank(self,  query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        for _ in range(self.num_repeat):
            ...
            while start_pos >= 0:
                ...
                window_before = ranking[start_pos: end_pos]
                result = self.compare(query, window_before)
                ranking = receive_permutation(ranking, result, start_pos, end_pos)
                window_after = ranking[start_pos: end_pos]

                self.last_compare_trace.append({
                    "rank_start": start_pos + 1,  # 1-based inclusive
                    "rank_end": end_pos,          # 1-based inclusive
                    "window_size": len(window_before),
                    "window_docids_before": [d.docid for d in window_before],
                    "window_docids_after": [d.docid for d in window_after],
...
class ListwiseLlmRanker(OpenAiListwiseLlmRanker):
    ...
    def __init__(self, model_name_or_path, tokenizer_name_or_path, device, window_size, step_size,
        ...
        self._vllm_llm = None  # populated below if use_vllm=True
        ...
        if self.config.model_type == 't5':
        ...
        elif self.config.model_type in ['llama', 'mistral', 'qwen3', 'qwen3_5', 'gemma3', 'gemma3_text', 'qwen3_moe']:
            ...
            if use_vllm:
                ...
                print(
                    f"[ListwiseLlmRanker] Loading '{model_name_or_path}' via vLLM "
                ...
                self.llm = None  # HF model not loaded
            else:
            ...
            # Suppress "attention_mask not set" warning (only needed for HF path)
            if not use_vllm and (self.tokenizer.pad_token_id is None or \
    ...
    def compare(self, query: str, docs: List):
        ...
        if self.scoring == 'generation':
            if self.config.model_type == 't5':
                ...
                if self.config.model_type in ['qwen3', 'qwen3_5', 'qwen3_moe']:
                    prompt = self.tokenizer.apply_chat_template(
                        input_text, tokenize=False, add_generation_prompt=True, enable_thinking=False
                    )
                else:
                    prompt = self.tokenizer.apply_chat_template(
                ...
                if self._vllm_llm is not None:
                    # ---- vLLM path: chat() + chat_template_kwargs ----
                    # This is the official Qwen3 pattern for disabling thinking;
                    # chat_template_kwargs is silently ignored by other models.
                    from vllm import SamplingParams as _SP
                    _out = self._vllm_llm.chat(
                        [input_text],
                        _SP(temperature=0.0, max_tokens=200),
                        chat_template_kwargs={"enable_thinking": False},
                        use_tqdm=False,
                    )[0]
                    self.total_prompt_tokens   += len(_out.prompt_token_ids)
                    self.total_completion_tokens += len(_out.outputs[0].token_ids)
                    output = _out.outputs[0].text.strip()
                else:
                    # ---- HF path (SDPA-accelerated) ----
